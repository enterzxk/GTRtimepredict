...
def train_single_task_stg(
    ...
    print(f"开始训练 SingleTaskSTGTransformer (device={device})")
    ...
    if best_bucket_report is not None:
        save_bucket_report(bucket_report_path, "SingleTaskSTG_Best", best_bucket_report)
...
if __name__ == "__main__":
    ...
    MODEL_SAVE_PATH = f"{SAVE_DIR}/best_stg_single.pth"
    REPORT_DIR = "results"
    BUCKET_REPORT_PATH = f"{REPORT_DIR}/stg_single_bucket_metrics.csv"

    TRAIN_SPLIT_RATIO = 0.8
    SPLIT_STRATEGY = "row"  # row | case
    MAX_SEQ_LENGTH = 50
    MAX_PREFIXES = 100
